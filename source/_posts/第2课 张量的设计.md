...
tags:
  ...
  - "学习笔记"
  - "课程"
description: "围绕 Tensor 的数据布局、形状变换、广播与内存管理，建立 KuiperInfer 张量实现的核心心智模型。"
readmore: true
mathjax: true
...
![01-reshape-element-count.png](/images/inference-framework/01-reshape-element-count.png)
...
- [ ] 理解 Tensor 的数据、形状和接口。
- [ ] 掌握不同维度到 `arma::fcube` 的映射。
- [ ] 理解 Fill、Transform、Reshape、Flatten 与 Padding。
...
![三维 Tensor 中 channel、row、col 的元素访问](https://pic2.zhimg.com/v2-fe79b61594b63bfb2acbc6957bfc9f13_1440w.jpg)
...
![Flatten 将多维 Tensor 展开成一维向量](https://i-blog.csdnimg.cn/blog_migrate/8897f8d941fd83fb1b50430ffa65adb6.png)
...
![Padding 在 Tensor 四周填充指定数值](https://i-blog.csdnimg.cn/blog_migrate/54394bdf2b59cd9879bcf70ad7696f40.png)
...
**程序：** `course2/kuiper_datawhale_course2`  
**结果：** 14 个 Tensor 构造、填充、变形、Flatten、Padding 与 Transform 测试全部通过。

![course2-tests-vscode.png](/images/inference-framework/course2-tests-vscode.png)

> **验证结论**
> Tensor 的逻辑形状、底层数据布局和各项变换操作保持一致。
...
| --- | --- | --- | --- |
| raw_shapes_ | 保存逻辑维度 | 区分 1D/2D/3D | 🟡 |
| arma::fcube | 保存三维浮点数据 | 底层存储 | 🟡 |
| 行/列主序 | 线性内存遍历顺序 | 数据填充 | 🟡 |
| Reshape | 改形状但总数不变 | 维度变换 | 🟡 |
| Padding | 四周填充值 | 尺寸处理 | 🟡 |
...
> 第 1 课的 Tensor 定位，以及 C++ class、vector、reference 和 const。

**在项目中的位置：**

> Tensor 是 RuntimeOperand 和 Layer 共同使用的基础数据类型。

---

## ⚠️ 易错点

### 1. 把 raw_shapes_ 当成数据存储。
...
> 必须根据 row_major 转置或重排。

---

## ❓ 待解决问题

- [ ] channels 为 1 时为什么可能是一维或二维？
- [ ] slice 为什么返回 const reference？
- [ ] Padding 如何避免 channel 数据错位？

## 📝 课后自测

### Q1｜画出 `[C,R,W]` 与 `fcube(R,W,C)` 的对应。

> 

### Q2｜解释为何 `2×3×4` 能变成 `24`，不能变成 `5×5`。

> 

### Q3｜独立实现 Flatten 并口述 Padding。
...
1. Tensor 包含逻辑形状、实际数据和接口。
2. fcube 承载数据，raw_shapes_ 保留逻辑维度。
3. 变形必须维护数量和顺序。

**一句话总结：**

> Tensor 的核心是让逻辑形状与底层连续数据始终一致。
