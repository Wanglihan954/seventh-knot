...
tags:
  ...
  - "学习笔记"
  - "课程"
description: "梳理 RuntimeGraph 的构建过程：解析 PNNX、建立拓扑关系、初始化张量并准备按序执行。"
readmore: true
mathjax: true
date: 2026-08-31 20:00:00
updated: 2026-09-18 16:54:40
abbrlink: "5d9c34a6"
---
> **课程信息**
> **课程：** 自制深度学习推理框架  
...
![01-topological-checkpoint.png](/images/inference-framework/01-topological-checkpoint.png)
...
- [ ] 理解 DAG 拓扑顺序和 DFS 逆后序。
- [ ] 掌握 RuntimeGraph 状态变化。
- [ ] 理解节点连接和输出张量预分配。
...
![简单计算图中的前驱节点与拓扑顺序](https://pica.zhimg.com/v2-37ed6844956354c8bad07d70723a1794_1440w.jpg)
...
![包含分支与汇合的计算图拓扑排序](https://pic4.zhimg.com/v2-e20ad84bd6a3b0c9198388b30766756f_1440w.jpg)
...
![通过 output_names 建立 RuntimeOperator 后继关系](https://pica.zhimg.com/v2-a14969fc78f3a82ffa8a6831f1fc546a_1440w.jpg)
...
![Build 阶段预分配各节点输出 Tensor](https://picx.zhimg.com/v2-924735125d79fdadc5f8191a00080e7f_1440w.jpg)
...
#### RuntimeOperatorUtils 的完整职责

可以把它记为：

> **Runtime 的 Tensor 资源准备与复用工具。**

它不负责 Conv、ReLU 等数学计算，也不负责解析 PNNX 文件。它把已经建立好的图推进到“可以直接执行”的状态：

```text
RuntimeOperand
= 描述需要什么数据（name / shape / dtype / datas）

RuntimeOperatorUtils
= 决定这些数据空间怎样准备和连接
...
- `InitOperatorInput()` 让当前节点的输入引用前驱节点已经拥有的输出 Tensor，并保持输入顺序；它通常不复制整块特征数据。
- `InitOperatorOutput()` 根据输出 `shape`、`dtype` 和 batch 创建输出 `RuntimeOperand` 与 Tensor 空间。
- 构建阶段完成这些工作，可以避免每次 `Forward()` 都反复申请、释放大块内存。
...
Conv1.output ───────────────┐
                           ├─ ReLU.input
同一组 shared_ptr<Tensor> ─┘
```

这里“复用输入”指的是上下游节点共享同一份中间结果，而不是重新复制一份。更进一步的工业 Runtime 还会做生命周期分析：当某个中间 Tensor 已经没有后续消费者时，把它的 buffer 分配给尺寸兼容的新输出。这属于内存规划优化，不能简单等同于当前课程版本已经完成的所有能力。
...
因此性能测试时不能把 `Init()`、`Build()`、模型文件读取和图片预处理混进单次前向耗时。
...
**程序：** `course4/kuiper_datawhale_course4`  
**结果：** 5 个拓扑排序、图连接、GraphState 和输出 Tensor 构建测试全部通过。

![course4-tests-vscode.png](/images/inference-framework/course4-tests-vscode.png)

> **验证结论**
> RuntimeGraph 已经从孤立节点构建为具有后继关系、输出空间与合法执行顺序的运行时图。
...
| --- | --- | --- | --- |
| 拓扑排序 | 前驱先于后继 | 生成执行顺序 | 🟡 |
| 逆后序 DFS | 回退时入队并逆序 | ReverseTopo | 🟡 |
| GraphState | 约束构建阶段 | 避免非法调用 | 🟡 |
| output_operators | 保存后继映射 | 恢复连接 | 🟡 |
| 输出预分配 | 提前创建 Tensor | 降低运行开销 | 🟡 |
...
> 第 3 课已生成 Runtime 节点、操作数、参数和权重。

**在项目中的位置：**

> Build 后 RuntimeGraph 才拥有连接、数据空间与合法执行顺序。
...
### Q2｜解释 ReverseTopo 为何最后 reverse。

> 

### Q3｜口述 Build 的连接和预分配过程。
...
> Build 把解析后的节点变成拥有连接、内存和执行顺序的运行时图。
